...
The STFT is computed using a block size of 512 samples with 50\% overlapping blocks; a Hann window is applied to each block. Only the magnitude spectrogram is used and the phase information is discarded. The resulting spectrogram $M_\mathrm{stft}$ is a $m \times n$ matrix, in which $m = 257$ and $n$ equals the number of blocks. This input representation has been compared in the literature related to SC as a baseline input \cite{Su2014Guitar, Su2014Violin}.  {\textcolor{red} {This confuses me a bit. This is the baseline comparison for the SC literature? With what has it been compared?}} 
...
\begin{figure}
    ...
    \caption{An example of (a) an IOI histogram vector $\mathbf{v}_\mathrm{ioi}$ and (b) an amplitude histogram vector $\mathbf{v}_\mathrm{amp}$. All histograms are normalized by the total number of counts and can be interpreted as probability distributions.{\textcolor{red}{maybe scale the axes to the values you mention in the text, not just use indices?}}} 
