...
\begin{table}
\centering
\begin{tabularx}{0.48 \textwidth}{cccc}
%\begin{tabular}{cccc}
\hline
...
\item \textbf{Designed}: the designed rhythmic and dynamic features derived from the IOI and amplitude histograms. These features involves the calculation of various statistics, such as crest, skewness, flatness, kurtosis, etc., directly from the histograms of the entire recording (See \cite{Wu2016} for more details). $d_{designed} = 18$ 
%{\color{red} we could add more description here. Maybe it's a bit annoying to look this all up?}

\item \textbf{SC (LHM)}: the SC learned features as described in Sect.~\ref{subsec:feat_learn} %{\color{red}why is this ``proposed'' and not lhm?}
